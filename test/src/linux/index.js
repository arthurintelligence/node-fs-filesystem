  ...
  describe('native tests', ifElse(
    ...
    () => function(){
      it('should properly parse the output of the filesystem command', function(done){
        ...
        const firstParse = linux.parser(userFilter)(input).devices;
        const devices = inferDevSize(firstParse);
        expect(devices).to.be.an('object');
        expect(Object.keys(devices).length).to.be.at.least(1);
        for(const k in devices){
          expect(devices[k].id).to.be.a('string');
          expect(devices[k].node).to.be.a('string');
          expect(devices[k].size).to.be.a('number').that.is.gte(0);
          expect(devices[k].volumes).to.be.an('array');
          devices[k].volumes.forEach((v) => {
