...
describe('windows integration tests', function() {
  describe(
    'native tests',
    ifElse(
      eqeqeq('win32'),
        ...
        function() {
          it('should properly parse the output of the filesystem command', function(done) {
            ...
            for(let k in acc.devices) {
              ...
              expect(acc.devices[k].size)
                .to.be.a('number')
                .that.is.gte(0);
              expect(acc.devices[k].description).to.be.a('string');
              expect(acc.devices[k].removable).to.be.a('boolean');
              expect(acc.devices[k].volumes).to.be.an('array').that.is.not
                .empty;
              acc.devices[k].volumes.forEach(v => {
                expect(typeof v.name === 'string' || v.name === null).to.be
                  .true;
      ...
      () =>
        function() {
          it('should not run native windows tests', done => done());
        }
    )(os.platform())
  );
